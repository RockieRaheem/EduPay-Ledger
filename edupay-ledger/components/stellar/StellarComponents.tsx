...
export function StellarAnchorIndicator({
  ...
  if (!anchor) {
    return (
      <span className="text-sm text-slate-400 italic">Not on blockchain</span>
    );
  }
    ...
    <div className="flex flex-col gap-2">
      ...
      {showDetails && anchor && (
        <div className="text-xs text-slate-500 space-y-1">
          <p>
            Attempts: {anchor.attempts}/{anchor.maxAttempts}
          </p>
          {anchor.anchoredAt && (
      ...
      <Card className={className}>
        ...
        <div className="mt-4 p-4 bg-yellow-50 rounded-lg">
          ...
          <p className="mt-2 text-sm text-yellow-700">
            Stellar blockchain integration is not configured. Add
            STELLAR_ANCHOR_SECRET_KEY and NEXT_PUBLIC_STELLAR_ANCHOR_PUBLIC_KEY
  ...
  return (
    <Card className={className}>
      ...
      <div className="flex items-center gap-2 mb-4">
        <span
          className={`w-3 h-3 rounded-full ${healthStatus?.isConnected ? "bg-green-500" : "bg-red-500"}`}
        />
      ...
      {healthStatus?.accountBalance !== undefined && (
        <div className="text-sm text-slate-600 mb-4">
          ...
          <span className="font-mono font-medium">
            {healthStatus.accountBalance.toFixed(2)} XLM
          </span>
        </div>
      )}
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Blockchain Verification">
      <div className="space-y-4">
        ...
        <div className="text-xs text-slate-500 text-center">
          <p>
            This payment record is cryptographically secured on the Stellar
            blockchain, providing an immutable, tamper-proof audit trail.
          </p>
        </div>
      </div>
    </Modal>
  ...
  const getIcon = () => {
    ...
    if (isPending)
      return {
        ...
        title: "Pending blockchain record",
      };
    if (isFailed)
      return {
        icon: "error",
        color: "text-red-500",
        title: "Blockchain record failed",
      };
    return null;
  };
