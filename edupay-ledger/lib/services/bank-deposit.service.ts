 ...
 */
export async function getBankAccounts(schoolId: string): Promise<SchoolBankAccount[]> {
  // Mock implementation
 ...
 */
export async function getDefaultBankAccount(schoolId: string): Promise<SchoolBankAccount | null> {
...
export async function createDepositSlip(
  ...
  const slipNumber = generateSlipNumber(settings.slipPrefix, settings.currentYear, nextNumber);
  
  // Would fetch actual payment details
  const payments: DepositPaymentReference[] = input.paymentIds.map((id, index) => ({
    paymentId: id,
 ...
 */
export async function getDepositSlip(slipId: string): Promise<BankDepositSlip | null> {
 ...
 * Query deposit slips
 */
export async function queryDepositSlips(query: DepositSlipQuery): Promise<BankDepositSlip[]> {
...
export async function addCheques(
  ...
  const totalAmount = [...existingCheques, ...cheques].reduce((sum, c) => sum + c.amount, 0);
 ...
 */
export async function markReadyForDeposit(slipId: string): Promise<BankDepositSlip> {
  const slip = await getDepositSlip(slipId);
...
export async function confirmBankReceipt(
  ...
  return updateDepositSlip(slipId, {
    ...
    hasDiscrepancy,
    discrepancyAmount: hasDiscrepancy ? slip.depositAmount - confirmedAmount : undefined,
  });
}
...
export async function startReconciliation(
  ...
  const items = slips.map(slip => ({
    ...
    status: slip.status === 'reconciled' ? 'matched' as const : 
            slip.hasDiscrepancy ? 'discrepancy' as const : 'pending' as const,
  ...
  const reconciliation: DepositReconciliation = {
    ...
    cashDeposits: slips.filter(s => s.paymentType === 'cash').reduce((sum, s) => sum + s.depositAmount, 0),
    chequeDeposits: slips.filter(s => s.paymentType === 'cheque').reduce((sum, s) => sum + s.depositAmount, 0),
  ...
  return [
    {
      ...
      paymentType: 'cash',
      includesPayments: [
        { paymentId: 'p1', receiptNumber: 'RCP-001', studentName: 'David Mugisha', amount: 500000, paymentDate: baseDate, paymentMethod: 'cash' },
        { paymentId: 'p2', receiptNumber: 'RCP-002', studentName: 'Grace Nakato', amount: 750000, paymentDate: baseDate, paymentMethod: 'cash' },
        { paymentId: 'p3', receiptNumber: 'RCP-003', studentName: 'Peter Ochen', amount: 1250000, paymentDate: baseDate, paymentMethod: 'cash' },
      ],
      denominations: {
        notes_50000: 40,
        notes_20000: 20,
...
function getMockDepositSummary(schoolId: string, period: 'daily' | 'weekly' | 'monthly'): DepositSummary {
  ...
  return {
    ...
    byBank: [
      { bank: 'stanbic', bankName: 'Stanbic Bank', count: 2 * multiplier, amount: 4500000 * multiplier },
      { bank: 'centenary', bankName: 'Centenary Bank', count: 1 * multiplier, amount: 1200000 * multiplier },
    ],
    pendingDeposits: 2,
